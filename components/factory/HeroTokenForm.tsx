...
const errorMessages: { [key: string]: (value: string, maxSupply?: string) => string } = {
  ...
  maxSupply: (value) => !value || isNaN(Number(value)) || Number(value) <= 0 ? "Max supply must be a positive number" : "",
  preMintAmount: (value, maxSupply) => {
    if (!value || isNaN(Number(value)) || Number(value) < 0) return "Pre-mint amount must be a non-negative number"
    if (maxSupply && Number(value) > Number(maxSupply)) return "Pre-mint amount cannot be larger than max supply"
    return ""
  },
  rewardsPerBlock: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Rewards per graffiti must be a non-negative number" : "",
  taxRate: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Tax rate must be a non-negative number" : "",
  crossChainFee: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Cross-chain fee must be a non-negative number" : "",
  lzGasLimit: (value) => !value || isNaN(Number(value)) || Number(value) <= 0 ? "LayerZero gas limit must be a positive number" : "",
  maxBonusRewardAfterOneDay: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Max bonus reward must be a non-negative number" : "",
...
export const HeroTokenForm = ({ onFormSubmit }: { onFormSubmit: (formData: HeroTokenFormType) => void }) => {
  ...
  const validateForm = (): boolean => {
    ...
    Object.keys(formData).forEach((key) => {
      if (key === 'isPro' || (!formData.isPro && ['treasury', 'feePayer', 'crossChainFee', 'lzGasLimit', 'owner'].includes(key))) {
        return;
      }
  ...
  const renderInput = (id: keyof HeroTokenFormType, label: string, placeholder: string, type = "text") => (
    <div>
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {renderInput("maxBonusRewardAfterOneDay", "Max Bonus Reward After One Day", "Enter max bonus reward after one day", "number")}
          ...
          {formData.isPro && (
            <>
              ...
              {renderInput("lzGasLimit", "LayerZero Gas Limit", "Enter layer zero gas limit", "number")}
          ...
          <Button onClick={handleCreate} className="w-full bg-muted hover:bg-muted/60">
            <Rocket className="mr-2 h-4 w-4 " /> <span className="text-yellow-500 font-bold">Create Token</span>
          </Button>
        </div>
      </CardContent>
    </Card>

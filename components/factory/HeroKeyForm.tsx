...
const errorMessages: { [K in keyof HeroKeyFormType]: (value: string, maxSupply?: string) => string } = {
  type: (value) => value.trim() == "key" ? "" : "Type must be key",
  isPro: (value) => value.trim() ? "" : "Type must be key",
  coinName: (value) => value.trim() ? "" : "Key name is required",
  ...
  baseCost: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Base cost must be a non-negative number" : "",
  costIncrement: (value) => !value || isNaN(Number(value)) || Number(value) < 0 ? "Cost increment must be a non-negative number" : "",
...
export const HeroKeyForm = ({ onFormSubmit }: { onFormSubmit: (formData: HeroKeyFormType) => void }) => {
  ...
  const validateForm = (): boolean => {
    ...
    Object.keys(formData).forEach((key) => {
      ...
      if (errorFunction && (formData.isPro || !['paymentToken', 'baseCost', 'costIncrement', 'lzGasLimit', 'treasury', 'feePayer', 'owner'].includes(typedKey))) {
        const errorMessage = errorFunction(typeof value === 'string' ? value : String(value), formData.maxSupply);
        if (errorMessage) {
  ...
  const renderInput = (id: keyof HeroKeyFormType, label: string, placeholder: string, type = "text") => (
    <div>
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          {renderInput("coinName", "Key Name", "Enter your key name")}
          ...
          <Button onClick={handleCreate} className="w-full bg-muted hover:bg-muted/60 ">
            <Rocket className="mr-2 h-4 w-4 " /> <span className="text-yellow-500 font-bold">Create Key</span>
          </Button>
        </div>
      </CardContent>
    </Card>
